...
class OrderControllerCore extends FrontController
{
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitReorder') && $id_order = (int) Tools::getValue('id_order')) {
            ...
            // } else {
                ...
                Tools::redirect('index.php?controller=order?id_reorder='.$id_order);
            // }
        }
    ...
    public function initContent()
    {
        ...
        //Проверка товара для вывода модального окна при перезаказе товара.
        if(Tools::getValue('id_reorder')) {
            $id_reorder = (int) Tools::getValue('id_reorder');

            $sql_question = "SELECT `id_cart`, `id_shop` FROM `" . _DB_PREFIX_ . "orders` WHERE `id_order` = " . $id_reorder;
            $id_cart = Db::getInstance()->executeS($sql_question);

            $sql_question = "SELECT `ps_cart_product`.`id_product`, `ps_cart_product`.`id_shop`, `ps_product_lang`.`name`, `ps_product`.`reference` FROM  `" . _DB_PREFIX_ . "cart_product` LEFT JOIN `ps_product_lang` ON `ps_product_lang`.`id_product` = `ps_cart_product`.`id_product` LEFT JOIN `ps_product` ON `ps_product`.`id_product` = `ps_cart_product`.`id_product` WHERE `id_cart` = " . $id_cart[0]['id_cart'] . " AND `ps_cart_product`.`id_shop` = " . $id_cart[0]['id_shop'] . " GROUP BY `ps_product_lang`.`name`";
            $cart_products = Db::getInstance()->executeS($sql_question);
            ...
            foreach ($cart_products as $cart_product) {
                ...
                if($product_available != 0) {
                    $sql_question = "SELECT `quantity` FROM `" . _DB_PREFIX_ . "stock_available` WHERE `id_product` = " . $cart_product['id_product'] ." AND `id_shop` = " . $cart_product['id_shop'];
                    $product_quantity = Db::getInstance()->getValue($sql_question);
                    if($product_quantity <= 0) {
                        $no_product[$quantity]['name'] = $cart_product['name'];
                        $no_product[$quantity]['reference'] = $cart_product['reference'];
                        $quantity++;
                    }
                } else {
                    $no_product[$available]['name'] = $cart_product['name'];
                    $no_product[$available]['reference'] = $cart_product['reference'];
                    $available++;
                }
            }

            if($available == 0 && $quantity == 0) {
                $text_message_start = 'Заказ создан успешно';
                $available_cycle = 0;
                $text_message_end = 'Проверьте корзину';
            } else {
                $text_message_start = 'Некоторых товаров сейчас нет в наличии';
                $available_cycle = 1;
                $text_message_end = 'Проверьте корзину';
            }
