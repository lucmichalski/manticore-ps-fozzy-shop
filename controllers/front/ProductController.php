...
class ProductControllerCore extends ProductPresentingFrontControllerCore
{
    ...
    public function initContent()
    {
        if (!$this->errors) {
            ...
            $this->context->smarty->assign('id_shop', (int)$this->context->shop->id);                     //Britoff - add shop
            $this->context->smarty->assign('id_shop_group', (int)$this->context->shop->id_shop_group);    //Britoff - add shop
           // dump($this->context->shop->id);
            ...
            //Britoff - add suppliers - start
            $sup=array();
              $sup=array();
              $sql_supp = "SELECT p.`id_supplier` FROM `" . _DB_PREFIX_ . "product_supplier` p, `" . _DB_PREFIX_ . "supplier` s WHERE p.`id_product` = ".$this->product->id." AND p.`id_supplier` = s.`id_supplier` AND s.`active` = 1";
              $suppliers = Db::getInstance()->executeS($sql_supp);
              
              foreach ($suppliers as $supplier) {
                $sup[]=(int)$supplier['id_supplier'];
              }
            //Britoff - add suppliers - stop
            
            //Britoff - actions for im - start
            $prices_im = array();
            $sql_prices_im = "SELECT * FROM `"._DB_PREFIX_."fozzy_autoupdate_online` WHERE `id_product` = ".$this->product->id." AND (NOW() BETWEEN `date_start` AND `date_stop`)";
            $prices_im = Db::getInstance()->executeS($sql_prices_im);
            $prices_im_act = count($prices_im);
         /*   if ($this->context->customer->id == 5)
            {
            if ($prices_im_act) dump($prices_im);
            }
         */   
            //Britoff - actions for im - stop
            
            //Britoff - product in cart - start
            $cart_quantity = 0;
            $cart_quantity_array = $this->context->cart->getProductQuantity($this->product->id);
            $cart_quantity = (float)$cart_quantity_array['quantity'];
            //Britoff - product in cart - stop
