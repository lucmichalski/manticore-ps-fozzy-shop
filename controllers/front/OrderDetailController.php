    ...
    public function initContent()
    {
        ...
        if (!$id_order) {
            ...
            if (Validate::isLoadedObject($order) && $order->id_customer == $this->context->customer->id) {
                ...
                $order_cart = (int)Db::getInstance()->getValue("SELECT `id_cart` FROM `" . _DB_PREFIX_ . "orders` WHERE `id_order` =" . $id_order);
                $order_cart_products_select = Db::getInstance()->executeS("SELECT * FROM `" . _DB_PREFIX_ . "cart_product` LEFT JOIN `" . _DB_PREFIX_ . "product_lang` ON `" . _DB_PREFIX_ . "product_lang`.`id_product`= `" . _DB_PREFIX_ . "cart_product`.`id_product` WHERE `id_cart` =" . $order_cart . " GROUP BY `ps_product_lang`.`id_product`");
              /*  $order_cart_products = array();
                foreach ($order_cart_products_select as $OCP)
                  {
                   $order_cart_products[$OCP['id_product']]=$OCP;
                  }           */
             //   $this->order_to_display['order_cart_product'] = $order_cart_products;
                $this->order_to_display['order_cart_product'] = $order_cart_products_select;
