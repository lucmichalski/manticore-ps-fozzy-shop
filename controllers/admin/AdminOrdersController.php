...
class AdminOrdersControllerCore extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $shops = explode("-",$this->context->cookie->shopContext);
        ...
        $select  = 'a.id_carrier, a.order_type, ';
        $select .= 'carr.`name` as carrier, a.`payment`, ';
        $select .= 'a.ice, a.norm, a.fresh, a.mest, a.fiskal, ';
        $select .= 'a.zone, a.zone_name, ';
        $select .= 'CASE WHEN c.`company` IS NOT NULL 
                    THEN CONCAT(LEFT(c.`firstname`, 1), \'. \', c.`lastname`, \' (\', c.`company`, \')\')
                    ELSE CONCAT(LEFT(c.`firstname`, 1), \'. \', c.`lastname`)
                    END AS customer, '; 
    		$select .= 'osl.`name` AS status_name, os.`color`, ';
    		$select .= ' CONCAT(logv.`fio`," - ",logv.`phone`) as vodila, logs.`fio` as sborshik, ';
   // 		$select .= '(SELECT CEILING(ocar.weight) FROM `'._DB_PREFIX_.'order_carrier` ocar WHERE ocar.id_order = a.id_order LIMIT 1) as order_weight, ';
        $select .= '(SELECT ROUND(SUM(`product_quantity`*`product_weight`),2) FROM `'._DB_PREFIX_.'order_detail` orcar WHERE orcar.`id_order` = a.`id_order` LIMIT 1) as order_weight, ';
            
 // Ваще нахуй  		$select .= '(SELECT COUNT(co.id_order) FROM `'._DB_PREFIX_.'orders` co WHERE co.id_customer = a.id_customer) as order_count, ';
        $select .= 'DATE_FORMAT(a.`dateofdelivery`, "%Y-%m-%d") as dated, ';
   //     $select .= 'DATE_FORMAT(dtd.`dateofdelivery`, "%Y-%m-%d") as dated, ';
        $select .= 'CONCAT(LEFT(dtd2.`timefrom`, 5),"-",LEFT(dtd2.`timeto`, 5)) as timed, ';  
        $select .= 'CONCAT(address.`city`,", ",address.`address1`) as address, address.`zone_name` as geo, ';
        $select .= 'address.`valid_adr` as valid_adr ';
        
        $join  = 'LEFT JOIN `'._DB_PREFIX_.'customer` c ON (c.`id_customer` = a.`id_customer`) ';
        $join .= 'LEFT JOIN `'._DB_PREFIX_.'carrier` carr ON (carr.`id_carrier` = a.`id_carrier`) ';
   //     $join .= 'LEFT JOIN `'._DB_PREFIX_.'fozzy_logistic` log ON (log.`id_order` = a.`id_order`) ';
        $join .= 'LEFT JOIN `'._DB_PREFIX_.'fozzy_logistic_sborshik` logs ON (logs.`id_sborshik` = a.`id_sborshik`) ';
        $join .= 'LEFT JOIN `'._DB_PREFIX_.'fozzy_logistic_vodila` logv ON (logv.`id_vodila` = a.`id_vodila`) ';
        $join .= 'LEFT JOIN `'._DB_PREFIX_.'address` address ON address.id_address = a.id_address_delivery ';
   ...
   //     $join .= 'LEFT JOIN `'._DB_PREFIX_.'nove_dateofdelivery_cart` dtd ON (dtd.`cart_id` = a.`id_cart`) ';
        $join .= 'LEFT JOIN `'._DB_PREFIX_.'nove_dateofdelivery` dtd2 ON (dtd2.`id_period` = a.`period`) ';
        ...
        /*
        $payment_array = array();
        $payment_array['Оплата наличными при получении']="Нал";  
        $payment_array['Оплата по безналичному расчету']="Безнал"; 
        $payment_array['Liqpay']="Кредитка";
        */
        $payment_array = array();
        $payment_array['ps_cashondelivery']="Нал";  
        $payment_array['ps_wirepayment']="Безнал"; 
        $payment_array['ecm_liqpay']="Кредитка";
         
        $carrier_array = array();
        $carriers = Carrier::getCarriers ($this->context->language->id, true, false, false, null, 0);
        foreach ($carriers as $carrier)
         {
          $carrier_array[$carrier['id_carrier']] =  $carrier['name'];
         }
        $carrier_array[41] =  'JST_OLD';
        $sql_vodila = 'SELECT `id_vodila`, `fio` FROM `'._DB_PREFIX_.'fozzy_logistic_vodila` WHERE `id_shop` = '.$id_shop." AND `deleted` = 0";
        $sql_sborshik = 'SELECT `id_sborshik`, `fio` FROM `'._DB_PREFIX_.'fozzy_logistic_sborshik` WHERE `id_shop` = '.$id_shop." AND `deleted` = 0";
        ...
        foreach ($vodily as $vodila) {
          $vodily_array[$vodila['id_vodila']] = $vodila['fio'];  
        }
        ...
        foreach ($links as $wind) {
       // dump($wind);
          $delivery_array[$wind['id_period']]=substr($wind['timefrom'],0,5)."-".substr($wind['timeto'],0,5)." ".$wind['carriers_name'];
        }
     //   dump($links);
        ...
        $this->fields_list = array(
            ...
            'order_type' => array(
                'title' => $this->trans('Тип', array(), 'Admin.Global'),
                'align' => 'text-center',
                'class' => 'fixed-width-xs'
            ),
            'mest' => array(
                'title' => $this->trans('Мест', array(), 'Admin.Global'),
                'align' => 'text-center',
                'class' => 'fixed-width-xs mest'
            ),
            'fiskal' => array(
                'title' => $this->trans('Чек', array(), 'Admin.Global'),
                'align' => 'text-center',
                'class' => 'fixed-width-xs fiskal'
            ),    
       /* Ваще нахуй     'order_count' => array(
        				'title' => $this->trans('Зак.', array(), 'Admin.Global'),
                'align' => 'text-right',
                'search' => false,
        				'havingFilter' => false,
    			  ),          */
            ...
            'total_paid_tax_incl' => array(
                'title' => $this->trans('Итого', array(), 'Admin.Global'),
                'align' => 'text-right',
                'type' => 'price',
                'currency' => false,
                'badge_success' => false,
            ...
            'valid_adr' => array(
                'title' => $this->trans('Адрес проверен', array(), 'Admin.Global'),
                'active' => 'status',
                'filter_key' => 'address!valid_adr',
                'align' => 'text-center',
                'type' => 'bool',
                'orderby' => false
            ),
            'address' => array(
      				  'title' => $this->trans('Адрес', array(), 'Admin.Global'),
                'search' => false,
            ),
            'phone_mobile' => array(
      				  'title' => $this->trans('Телефон', array(), 'Admin.Global'),
            //    'search' => false,
                'filter_key' => 'address!phone_mobile',
            ),
            'order_weight' => array(
        				'title' => $this->trans('Вес', array(), 'Admin.Global'),
                'search' => true,
        				'havingFilter' => true,
    			  ),
            'dated' => array(
      				'title' => $this->trans('Доставка', array(), 'Admin.Global'),
              'search' => true,
              'type' => 'date',
              'class' => 'dated',
      				'filter_key' => 'a!dateofdelivery'
            ),
            'timed' => array(
      				'title' => $this->trans('Время доставки', array(), 'Admin.Global'),
              'class' => 'timed',
              'type' => 'select',
              'list' => $delivery_array,
              'filter_key' => 'a!period',
            ...
            'norm' => array(
                'title' => $this->trans('С', array(), 'Admin.Global'),
                'active' => 'status',
                'filter_key' => 'a!norm',
                'align' => 'text-center',
                'type' => 'bool',
           ...
           'fresh' => array(
                'title' => $this->trans('О', array(), 'Admin.Global'),
                'active' => 'status',
                'filter_key' => 'a!fresh',
                'align' => 'text-center',
                'type' => 'bool',
            ...
            'ice' => array(
                'title' => $this->trans('З', array(), 'Admin.Global'),
                'active' => 'status',
                'filter_key' => 'a!ice',
                'align' => 'text-center',
                'type' => 'bool',
            ...
            'vodila' => array(
                'title' => $this->trans('Водитель', array(), 'Admin.Global'),
                'type' => 'select',
                'list' => $vodily_array,
                'filter_key' => 'a!id_vodila',
                'filter_type' => 'int',
                'order_key' => 'vodila',
            ),
            'sborshik' => array(
                'title' => $this->trans('Сборщик', array(), 'Admin.Global'),
                'type' => 'select',
                'list' => $sborshiky_array,
                'filter_key' => 'a!id_sborshik',
                'filter_type' => 'int',
                'order_key' => 'sborshik'
            ),  
            'geo' => array(
                'title' => $this->trans('Гео', array(), 'Admin.Global'),
                'search' => true,
        				'havingFilter' => true,
            ),
            'zone_name' => array(
                'title' => $this->trans('Зона сборки', array(), 'Admin.Global'),
                'search' => true,
        				'havingFilter' => true,
            ),                     
            'date_add' => array(
      ...
      /*   if ($this->context->employee->id == 275)  // Тестовый оператор
                {
                dump($this->context->employee->id_profile);
                die();
                }    */
        if ($this->context->employee->id_profile == 5)
        {
    ...
    public function setMedia($isNewTheme = false)
    {
      ...
      //  $this->addJS('https://maps.googleapis.com/maps/api/js?v=3.exp');
          $this->addJS('https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&key=AIzaSyDkDGRsca6tykh3NF4LzaBWFc8c3YDCOiw');
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('change_shop') && isset($order) && Tools::getValue('shop_to_go') > 0 )
          {
          $shop_to_go = Tools::getValue('shop_to_go');
          $sql_update_order = 'UPDATE `'._DB_PREFIX_.'orders` SET `id_shop` = '.$shop_to_go.' WHERE id_order = '.$order->id;
          $sql_update_order_detail = 'UPDATE `'._DB_PREFIX_.'order_detail` SET `id_shop` = '.$shop_to_go.' WHERE id_order = '.$order->id;
          $sql_update_cart = 'UPDATE `'._DB_PREFIX_.'cart` SET `id_shop`= '.$shop_to_go.' WHERE id_cart = '.$order->id_cart;
          $sql_update_cart_product = 'UPDATE `'._DB_PREFIX_.'cart_product` SET `id_shop`= '.$shop_to_go.' WHERE id_cart = '.$order->id_cart;
          $sql_update_customer = 'UPDATE `'._DB_PREFIX_.'customer` SET `id_shop`= '.$shop_to_go.' WHERE id_customer = '.$order->id_customer;
          $sql_update_customer_thread = 'UPDATE `'._DB_PREFIX_.'customer_thread` SET `id_shop`= '.$shop_to_go.' WHERE id_customer = '.$order->id_customer;
          Db::getInstance()->execute($sql_update_order);
          Db::getInstance()->execute($sql_update_order_detail);
          Db::getInstance()->execute($sql_update_cart);
          Db::getInstance()->execute($sql_update_cart_product);
          Db::getInstance()->execute($sql_update_customer);
          Db::getInstance()->execute($sql_update_customer_thread);
          $this->errors[] = $this->trans('Заказ перенесен. Не забудьте обновить страницу, изменить перевозчика, дату и окно доставки!', array(), 'Admin.Orderscustomers.Notification');
          }
            ...
            if ($this->access('edit')) {
                if (!Tools::getValue('discount_name')) {
                    ...
                    foreach ($cart_rules as &$cart_rule) {
                        ...
                        if ((int)Tools::getValue('discount_type') == 3) {
        ...
        } elseif (Tools::isSubmit('submitStateTSD') && Tools::getValue('id_order') > 0 && Tools::getValue('id_order_state') > 0 ) {
        
        if ($this->context->employee->id == 1)
                {
                // executes hook
                $id_order = Tools::getValue('id_order');
                $new_os = Tools::getValue('id_order_state');
                $mess = 'Обновление даных по заказу';
                $sql_log = "INSERT INTO `"._DB_PREFIX_."log`(`severity`, `error_code`, `message`, `object_type`, `object_id`, `id_employee`, `date_add`, `date_upd`) VALUES (1,0,'$mess','order',".$id_order.",".$this->context->employee->id.",NOW(),NOW())";
                ...
                $this->errors[] = $this->trans('Данные на ТСД обновлены.', array(), 'Admin.Notifications.Error');
                }
        } elseif (Tools::isSubmit('submitKievChange') && Tools::getValue('id_order') > 0 ) {

                // executes hook
                $id_order = Tools::getValue('id_order');
                $new_filial = (int)Tools::getValue('filial_to_go');
                
                $history = new OrderHistory();
                $history->id_order = $id_order;
                $history->id_employee = $this->context->employee->id;
                $history->changeIdOrderState(908, $id_order);
                $history->add();
                ...
                $sql_zone = "UPDATE `"._DB_PREFIX_."orders` SET `zone` = '".$new_filial."', `zone_name` = '".$order_zone_name."', `id_sborshik` = 0 WHERE `id_order` = ".$id_order;
                Db::getInstance()->execute($sql_zone);
                
                $history_after = new OrderHistory();
                $history_after->id_order = $id_order;
                $history_after->id_employee = $this->context->employee->id;
                $history_after->changeIdOrderState(913, $id_order);
                $history_after->add();
          
                $mess = 'Зона изменена на '.$order_zone_name;
                $sql_log = "INSERT INTO `"._DB_PREFIX_."log`(`severity`, `error_code`, `message`, `object_type`, `object_id`, `id_employee`, `date_add`, `date_upd`) VALUES (1,0,'$mess','order',".$id_order.",".$this->context->employee->id.",NOW(),NOW())";
                Db::getInstance()->execute($sql_log);

                $this->errors[] = $this->trans('Филиал сборки изменен.', array(), 'Admin.Notifications.Error');
        } elseif (Tools::isSubmit('submitVodilaChange') && Tools::getValue('id_order') > 0 ) {
          $id_order = Tools::getValue('id_order');                                                   
         ...
         // $comps_array_address = array('orderId'=>$id_order,'deliveryAddress'=>'ул. Пишка 245/4, кв. №34-а');
          ...
          $restt = $this->PostJSON($link_vodila,1,$comps_array_vodila_json);
        }
        
        parent::postProcess();
    }

    private function PostJSON($link, $post_get = 1,$jsonRequest = NULL) {
      $ch = curl_init();
    ...
    public function renderKpis()
    {
        ...
        //Новые заказы
        $sql_new_orders = 'SELECT `id_order` FROM `'._DB_PREFIX_.'orders` WHERE `current_state` = 912';
        if ($id_shop) $sql_new_orders .= ' AND `id_shop` = '.$id_shop;
        $new_orders = Db::getInstance()->executeS($sql_new_orders);
        //Заказы на согласовании
        $sql_s_orders = 'SELECT `id_order` FROM `'._DB_PREFIX_.'orders` WHERE `current_state` = 914';
        if ($id_shop) $sql_s_orders .= ' AND `id_shop` = '.$id_shop;
        $s_orders = Db::getInstance()->executeS($sql_s_orders);
        //Заказы в недозвоне
        $sql_nd_orders = 'SELECT `id_order` FROM `'._DB_PREFIX_.'orders` WHERE `current_state` = 935';
        if ($id_shop) $sql_nd_orders .= ' AND `id_shop` = '.$id_shop;
        $nd_orders = Db::getInstance()->executeS($sql_nd_orders);
        //Окна
        $today_dayname = date('D');
        $tomorow_dayname = date('D',strtotime("+1 day"));
        $today_weeknum = (int)date('W');
        $tomorow_weeknum = (int)date('W',strtotime("+1 day"));
        ...
        //$sql_windows_close = "SELECT ".$today_dayname." as plan, window FROM "._DB_PREFIX_."nove_dateofdelivery_block WHERE week = ".$today_weeknum;
        //if ($id_shop) $sql_windows_close.=' AND `id_shop` = '.$id_shop;
        
        $sql_windows_close = "SELECT SUM(".$today_dayname.") as plan, window FROM "._DB_PREFIX_."nove_dateofdelivery_block WHERE week = ".$today_weeknum." AND `window` IN (71,72,67,68,69,70,97,98,99,100,101,102,85,86,87,88,89,90,73,74,75,76,77,78,116,117,118,119,120,121,140,141,142,143,144,145) ";
        if ($id_shop) $sql_windows_close.=' AND `id_shop` IN ('.$shops_to_close.') GROUP BY `window` ORDER BY `window`';
        
        
        $windows_close = Db::getInstance()->executeS($sql_windows_close);
        
        foreach ($windows_close as $key=>$window) {
                     $sql_cart_w = "SELECT cart_id FROM "._DB_PREFIX_."nove_dateofdelivery_cart WHERE period = ".$window['window']." AND dateofdelivery = '".date ('Y-m-d')."'";                            
                     $cart_w = Db::getInstance()->executeS($sql_cart_w);
                     $sql_window_name = "SELECT CONCAT('c ',TIME_FORMAT(timefrom,'%H:%i'),' по ',TIME_FORMAT(timeto,'%H:%i')) as name FROM "._DB_PREFIX_."nove_dateofdelivery WHERE id_period = ".$window['window'];
                     $window_name = Db::getInstance()->executeS($sql_window_name); 
                   //  d($sql_window_name);
                     $carts = array();
                     foreach ($cart_w AS $cart)
                      ...
                      if ($carts) {
                      $sql_order_in_window = "SELECT COUNT(id_order) as odr FROM "._DB_PREFIX_."orders WHERE id_cart IN (".implode(",",$carts).") AND current_state IN (SELECT `id_order_state` FROM `"._DB_PREFIX_."order_state` WHERE `window` = 1) AND id_carrier IN (27,30,33,42,55,57)";
                      if ($id_shop) $sql_order_in_window.=' AND `id_shop` = '.$id_shop;
                      $order_count = Db::getInstance()->executeS($sql_order_in_window);
                      $windows_close[$key]['close'] = $order_count[0]['odr'];
                      }
                      $windows_close[$key]['name'] = $window_name[0]['name'];
                  }
     //   if ($this->context->employee->id == 1) dump($windows_close);
        $windows_close = Tools::ArrayOrderBy($windows_close,'name');
        unset($window);
        unset($key);
        
        //$sql_windows_close_t = "SELECT ".$tomorow_dayname." as plan, window FROM "._DB_PREFIX_."nove_dateofdelivery_block WHERE week = ".$tomorow_weeknum;
        //if ($id_shop) $sql_windows_close_t.=' AND `id_shop` = '.$id_shop;
        
        $sql_windows_close_t = "SELECT SUM(".$tomorow_dayname.") as plan, window FROM "._DB_PREFIX_."nove_dateofdelivery_block WHERE week = ".$tomorow_weeknum." AND `window` IN (71,72,67,68,69,70,97,98,99,100,101,102,85,86,87,88,89,90,73,74,75,76,77,78,116,117,118,119,120,121,140,141,142,143,144,145) ";
        if ($id_shop) $sql_windows_close_t.=' AND `id_shop` IN ('.$shops_to_close.') GROUP BY `window` ORDER BY `window`';
        
        $windows_close_t = Db::getInstance()->executeS($sql_windows_close_t);
      //  if ($this->context->employee->id == 1) dump($windows_close_t);
        foreach ($windows_close_t as $key=>$window) {
                     $sql_cart_w = "SELECT cart_id FROM "._DB_PREFIX_."nove_dateofdelivery_cart WHERE period = ".$window['window']." AND dateofdelivery = '".date ('Y-m-d',strtotime("+1 day"))."'";                            
                     $cart_w = Db::getInstance()->executeS($sql_cart_w);
                     $sql_window_name = "SELECT CONCAT('c ',TIME_FORMAT(timefrom,'%H:%i'),' по ',TIME_FORMAT(timeto,'%H:%i')) as name FROM "._DB_PREFIX_."nove_dateofdelivery WHERE id_period = ".$window['window'];
                     $window_name = Db::getInstance()->executeS($sql_window_name); 
                   //   if ($this->context->employee->id == 1) d($cart_w);
                     $carts = array();
                     foreach ($cart_w AS $cart)
                      {
                         $carts[]= $cart['cart_id']; 
                      ...
                      if ($id_shop) $sql_order_in_window.=' AND `id_shop` = '.$id_shop;
                      $order_count = Db::getInstance()->executeS($sql_order_in_window);
                      $windows_close_t[$key]['close'] = $order_count[0]['odr'];
                      }
                      $windows_close_t[$key]['name'] = $window_name[0]['name'];
                  }
        $windows_close_t = Tools::ArrayOrderBy($windows_close_t,'name');
    // if ($this->context->employee->id == 1) dump($windows_close);
        foreach ($windows_close as $key_close=>$wind_close)
        {
          $helper = new HelperKpi();
      		$helper->id = 'box-window'.$key_close;
      		$helper->icon = 'icon-cart-plus';
      		if ($wind_close['close'] >= $wind_close['plan']) $helper->color = 'color2';
      		else $helper->color = 'color3';
      		$helper->title = $wind_close['name'];
          $helper->subtitle = $wind_close['close']." / ".$windows_close_t[$key_close]['close'];
          $helper->value = $wind_close['plan']." / ".$windows_close_t[$key_close]['plan'];
        ...
        $helper = new HelperKpi();
    		...
    		$helper->icon = 'icon-cart-plus';
    		$helper->color = 'color1';
    		$helper->title = $this->l('Новые', null, null, false);
        $helper->value = count ($new_orders); 
    		$kpis[] = $helper->generate();
        
        //Заказы на согласовании
        $helper = new HelperKpi();
    		$helper->id = 'box-s-order';
    		$helper->icon = 'icon-cart-plus';
    		$helper->color = 'color4';
    		$helper->title = $this->l('На согласовании', null, null, false);
        $helper->value = count ($s_orders); 
    		$kpis[] = $helper->generate();
        
        //Заказы в недозвоне
        if ((int)count($nd_orders)>0) {
        $helper = new HelperKpi();
    		$helper->id = 'box-nd-order';
    		$helper->icon = 'icon-cart-plus';
    		$helper->color = 'color2';
    		$helper->title = $this->l('Недозвон', null, null, false);
        $helper->value = count ($nd_orders); 
    		$kpis[] = $helper->generate();
        }
    ...
    public function renderView()
    {
        ...
        foreach ($products as &$product) {
         ...
         $p_image_tag = 'https://img.fozzyshop.com.ua/img/p/'.implode("/",str_split($product['image']->id)).'/'.$product['image']->id.'-small_default.jpg.jpg';
         if ($this->context->employee->id == 1)
        {
     //   dump($p_image_tag);
     //   die();
        }
           $product['image_tag'] = '<img src="'. $p_image_tag .'" width="60px" />';
        ...
        $shipping_refundable_tax_incl = max(0, $shipping_refundable_tax_incl);
         ...
         $cur_st = $order->getCurrentOrderState();
         $sql_st_to_change = '
              SELECT *
              FROM `' . _DB_PREFIX_ . 'order_state` os
              WHERE deleted = 0  AND `changes` = 1 ';
             
         $states_to_change = Db::getInstance()->executeS($sql_st_to_change);
         $id_states_to_change = array();
         foreach ($states_to_change as $ss)
           {
            $id_states_to_change[]=$ss['id_order_state'];
           }
         if (in_array($cur_st->id, $id_states_to_change)) {
          $show_tsd_from = 1;
          }
         else
          {
        ...
        //Блок вывода статусов - старт
        $states_to_show = OrderState::getOrderStates($this->context->language->id, $this->context->employee->id == 1 ? 0:1);
        $opt_state_show = 1;
       ...
       //    die();
        ...
        if ($this->context->employee->id_profile == 5)
            {
             $states_to_show = array(
                                array('id_order_state' => 915, 'name' => 'Досборка'),
                                array('id_order_state' => 6, 'name' => 'Отменено'),
                                array('id_order_state' => 913, 'name' => 'Подтвержден')
                                );
           switch ((int)$cur_st->id) {
             case 912:
                 $states_to_show = array(
                                array('id_order_state' => 913, 'name' => 'Подтвержден'),
                                array('id_order_state' => 6, 'name' => 'Отменено')
             ...
             case 939:
                 $states_to_show = array(
                                array('id_order_state' => 16, 'name' => 'Готов к отгрузке'),
                                array('id_order_state' => 6, 'name' => 'Отменено')
                                );
                 break;
             case 922:
        ...
        $this->tpl_view_vars = array(
            ...
            'carrier' => new Carrier($order->id_carrier),
            'history' => $history,
            'states' => $states_to_show,
            'opt_state_show' => $opt_state_show,
            'show_tsd_from' => $show_tsd_from,
    ...
    public function ajaxProcessAddProductOnOrder()
    {
        ...
        //Britoff - не даем сделать дубль
        if ($order->orderContainProduct($product_informations['product_id'])) {
            die(json_encode(array(
                'result' => false,
                'error' => $this->trans('Этот товар уже есть в заказе!', array(), 'Admin.Orderscustomers.Notification'),
            )));
        }
        ...
        //$order = $order->refreshShippingCost(); //Britoff - убрал дичь с тормозоми
    ...
    public function ajaxProcessEditProductOnOrder()
    {
     ...
     /*   // Apply change on OrderInvoice
        if (isset($order_invoice)) {
            ...
            die(json_encode(array(
                'result' => false,
                'error' => $this->trans('The order object cannot be loaded4.', array(), 'Admin.Orderscustomers.Notification').$old_order_invoice->total_products,
            )));
        }
      ...
      */  
        ...
        // Update product available quantity  -- Britoff - заблокировал обновление на складе, изза ошибок с большим кол-вом товара
...
//{
        //Britoff - оверсумма
        $blocked_summ = (float)Db::getInstance()->getValue("SELECT `amount` FROM `"._DB_PREFIX_."order_payment` WHERE `order_reference` = '".$order->reference."'");
        $order_type_payment = Db::getInstance()->getValue("SELECT `payment_method` FROM `"._DB_PREFIX_."order_payment` WHERE `order_reference` = '".$order->reference."'");
        if ($order_type_payment == 'Liqpay' && $blocked_summ < $order->total_paid) {
            die(json_encode(array(
                'result' => false,
                'error' => $this->trans('Сумма заказа превышает сумму, оплаченную клиентом: '.$blocked_summ." грн.", array(), 'Admin.Orderscustomers.Notification'),
            )));
        }
//}
