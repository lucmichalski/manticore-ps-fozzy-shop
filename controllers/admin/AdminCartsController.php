...
class AdminCartsControllerCore extends AdminController
{
    public function __construct()
    {
        ...
        //Britoff -----------------   Change User in Cart
        if ( Tools::getValue('ajaxchange') && Tools::getValue('user_id') ) {
            $c_userid = (int)Tools::getValue('user_id');
            $c_carid = (int)Tools::getValue('id_cart');
            $sql_uc = "UPDATE "._DB_PREFIX_."cart SET id_customer = ".$c_userid." WHERE id_cart = ".$c_carid;
            Db::getInstance()->executeS($sql_uc);
        } 
        //Britoff -----------------
        ...
        if ($this->access('edit')) {
            ...
            if (isset($product) && $product->id) {
                if (($id_product_attribute = Tools::getValue('id_product_attribute')) != 0) {
                    if (!Product::isAvailableWhenOutOfStock($product->out_of_stock) && !Attribute::checkAttributeQty((int) $id_product_attribute, (float) $qty)) {
                        $errors[] = $this->trans('There are not enough products in stock.', array(), 'Admin.Catalog.Notification');
                    }
                } elseif (!$product->checkQty((float) $qty)) {
                    $errors[] = $this->trans('There are not enough products in stock.', array(), 'Admin.Catalog.Notification');
                }
