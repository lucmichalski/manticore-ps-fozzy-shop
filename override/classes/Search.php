...
{
    protected static $keybords = array(
        'q' => 'й', 'w' => 'ц', 'e' => 'у', 'r' => 'к', 't' => 'е', 'y' => 'н', 'u' => 'г', 'i' => 'ш',
        'o' => 'щ', 'p' => 'з', '[' => 'х', ']' => 'ъ', '{' => 'Х', '}' => 'Ъ', '`' => 'ё', '~' => 'Ё',
        'a' => 'ф', 's' => 'ы', 'd' => 'в', 'f' => 'а', 'g' => 'п', 'h' => 'р', 'j' => 'о', 'k' => 'л',
        'l' => 'д', ';' => 'ж', ':' => 'Ж', "'" => 'э', '"' => 'Э', 'z' => 'я', 'x' => 'ч', 'c' => 'с',
        'v' => 'м', 'b' => 'и', 'n' => 'т', 'm' => 'ь', ',' => 'б', '.' => 'ю', '<' => 'Б', '>' => 'Ю',
        '?' => ',', '/' => '.',
    );
    protected static $keybordsRu = array(
        'кс' => 'x', 'ир' => 'eer', 'ко' => 'co', 'ак' => 'ac', 'фе' => 'ffee', 'ике' => 'icke',
        'а' => 'a', 'б' => 'b', 'в' => 'v',
        'г' => 'g', 'д' => 'd', 'е' => 'e',
        'ё' => 'e', 'ж' => 'j', 'з' => 'z',
        'и' => 'i', 'й' => 'y', 'к' => 'k',
    ...
    protected static $keybordsUaRu = array(
        'і' => 'е', 'ї' => 'и',
    );

    public static function getKeybords() {
        return self::$keybords;
    }
    public static function getKeybordsRu() {
        return self::$keybordsRu;
    }
    ...
    )
    {
        ...
        if (is_numeric($_REQUEST['s']) && $_REQUEST['s'] < 999999) {
            ...
            if (false !== $sphinx_results) {
                if (empty($sphinx_results['total']) && preg_match('![a-z]!i', $search_text)) {
                    $sphinx_results = self::getSphinxResults($id_lang, strtr($search_text, self::$keybords), $start, $page_size, $context);
                }
                if (empty($sphinx_results['total'])) {
                    $sphinx_results = self::getSphinxResults($id_lang, $expr, $start, $page_size, $context);
                }
                if (empty($sphinx_results['total']) && preg_match('![а-я]!i', $search_text)) {
                    $sphinx_results = self::getSphinxResults($id_lang, strtr($search_text, self::$keybordsRu), $start, $page_size, $context);
                }
            }
            if (empty($sphinx_results['total'])) {
    ...
    protected static function getRequestString($string)
    {
        ...
        $s = mb_eregi_replace('[^\d\sa-zа-яєіїё]+', '', $s);
        // Удаляет еденичные символы
        $s = preg_replace('! . !', ' ', $s . ' ');
    ...
    public static function getSphinxResults($id_lang = 1, $search_query, $page_number=1, $page_size=1, Context $context = null)
    {
        ...
        if ($link) {
            ...
            $query = 'SELECT *, ' . pow(2,$context->shop->id-1) . ' & available_for_order AS `sort`, WEIGHT() AS `weight`'
                . ' FROM `products' . (int)$id_lang . '`'
                . ' WHERE MATCH(\'@(product,category,manufacture) ' . $match . '\')'
                . ' AND shopId_attr=' . (int)$context->shop->id
                //. ' AND langId_attr=' . (int)$id_lang # язык указан в FROM
                . ' ORDER BY `sort` DESC, `weight` DESC, `price` ASC'
                //.' LIMIT '.$page_number.', '.$page_size # листалка в SQL запросе выше
                . ' LIMIT 0, 10000'
                . ' OPTION max_query_time=10'
                . ', field_weights=(manufacture=3, category=2, product=1)'
...
/*          категории берутся из списка товаров
            $query = 'SELECT *, WEIGHT() AS `weight` FROM `categories`'
                . ' WHERE MATCH(\'' . $match . '\')'
                . ' AND shopid_attr=' . (int)$context->shop->id
