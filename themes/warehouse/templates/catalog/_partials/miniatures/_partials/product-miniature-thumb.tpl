    ...
    <div class="thumbnail-container">
        ...
        {block name='product_flags'}
            <ul class="product-flags">
                {foreach from=$product.flags item=flag}
                    <li class="product-flag {$flag.type}">{if $flag.type != 'on-sale'}{$flag.label}{/if}
        ...
        {block name='product_flags'}
            <ul class="product-icons">
                    ...
                    <li class="product-icon">
                        <img src="/img/su/{$supplier}-icon_default.jpg">
                    </li>
                    {/if}
                   {/foreach}
            </ul>
        ...
        {block name='product_availability'}
            <div class="product-availability d-block">
                ...
                <span class="badge product-available mt-2">             {if isset($product.availability_message) && $product.availability_message}{$product.availability_message}{else}{l s='Available' d='Shop.Warehousetheme'}{/if}</span>
                    {elseif (isset($product.quantity_all_versions) && $product.quantity_all_versions > 0)}
                    <span class="badge product-available">             {if isset($product.availability_message) && $product.availability_message}{$product.availability_message}{else}{l s='Product available with different options' d='Shop.Warehousetheme'}{/if}</span>
                    {else}
                    <span class="badge product-unavailable">{if isset($product.availability_message) && $product.availability_message}{$product.availability_message}{else}{l s='Out of stock' d='Shop.Warehousetheme'}{/if}</span>
                    {/if}
