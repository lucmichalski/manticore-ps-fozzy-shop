...
{block name="override_tpl"}
  ...
  {if $opt_state_show == 0}
  <div class="rb-overlay" style="opacity: 0.5;position:fixed;top:0;left:0;width:100%;height:100%;z-index:500;background-color:#248e34;"></div>
  {/if}
  ...
  <div class="row">
    <div class="col-lg-7">
      <div class="panel">
        <div class="panel-heading">
          <i class="icon-credit-card"></i>{$order->order_type}
         <!-- {l s='Order' d='Admin.Global'} -->
      <!--    <span class="badge">{$order->reference}</span>     -->
          <span class="badge">{l s="#" d='Admin.Orderscustomers.Feature'}{$order->id}, Корзина {$order->id_cart} , {if $order->order_from} к заказу {$order->order_from}{/if}</span>
        ...
        <div class="well hidden-print">
          {if Configuration::get('PS_INVOICE') && count($invoices_collection) && $order->invoice_number}
            <a data-selenium-id="view_invoice" class="btn btn-default _blank" href="{$link->getAdminLink('AdminPdf', true, [], ['submitAction' => 'generateInvoicePDF', 'id_order' => $order->id|intval])|escape:'html':'UTF-8'}">
              <i class="icon-file"></i>
              {l s='Лист сборки' d='Admin.Orderscustomers.Feature'}
            </a>&nbsp;
            <a class="btn btn-default _blank"  href="{$link->getAdminLink('AdminPdf', true, [], ['submitAction' => 'generateDeliverySlipPDF', 'id_order' => $order->id|intval])|escape:'html':'UTF-8'}">
              ...
              {l s='Счет для безнала' d='Admin.Orderscustomers.Feature'}
            </a>
          {else}
          ...
          {/if}
          &nbsp;
          <a id="Export-to-Excel" class="btn btn-default" href="/modules/priceexcel/export.php?id_order={$order->id}">{l s='Экспорт в Excel' d='Admin.Orderscustomers.Feature'}</a>
       ...
       <!--     <form action="{$currentIndex|escape:'html':'UTF-8'}&amp;vieworder&amp;token={$smarty.get.token}" method="post" class="form-horizontal well hidden-print">
              <div class="row">
              <input type="hidden" name="id_order" value="{$order->id}" />
              <input type="hidden" name="id_order_state" value="{$currentState->id}" />
                <div class="col-lg-3">
                  <button type="submit" name="submitStateTSD" id="submit_state_tsd" class="btn btn-primary">
                    {l s='Обновить на ТСД' d='Admin.Orderscustomers.Feature'}
                  </button>
                </div>
              </div>
            </form>     -->
      ...
      <div class="panel">
        {if $customer->id}
          ...
          <div class="row">
            ...
            <div class="col-xs-6"> 
            {if ($order->id_shop_group == 1 && $employee == 1)}
            <div class="panel panel-sm">
                <div class="panel-heading">
                  <i class="icon-change"></i>
                  {l s='Перенос заказа в другой магазин' d='Admin.Orderscustomers.Feature'}
                </div>
                <form id="change_shops" class="form-horizontal" method="post" action="{$link->getAdminLink('AdminOrders', true, [], ['vieworder' => 1, 'id_order' => $order->id|intval])|escape:'html':'UTF-8'}">
                  <div class="form-group">
                    <div class="col-lg-12">
                    <select name="shop_to_go" id="shop_to_go" class="form-control fixed-width-xl">
                    {foreach from=$shops key=k item=v}
                       <option value="{$k}" {if $order->id_shop == $k}selected{/if}>{$v.name}</li>
                    {/foreach}
                    </select>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-lg-12">
                      <button type="submit" id="change_shop" name="change_shop" class="btn btn-default pull-right">
                        <i class="icon-save"></i>
                        {l s='Перенести' d='Admin.Actions'}
                      </button>
                    </div>
                  </div>
                </form>
              ...
              {/if}  
            {if ($order->id_shop_group == 1 && $order->id_shop == 1 && ($employee == 1 || $employee == 10 || $employee == 34 || $employee == 172 || $employee == 276 || $employee == 281))}
            <div class="panel panel-sm">
                <div class="panel-heading">
                  <i class="icon-change"></i>
                  {l s='Изменение филиала сборки' d='Admin.Orderscustomers.Feature'}
                </div>
                <form id="change_filial" class="form-horizontal" method="post" action="{$link->getAdminLink('AdminOrders', true, [], ['vieworder' => 1, 'id_order' => $order->id|intval])|escape:'html':'UTF-8'}">
                  <div class="form-group">
                    <div class="col-lg-12">
                    <select name="filial_to_go" id="filial_to_go" class="form-control fixed-width-xl">
                       <option value="0" {if $order->zone == '0'}selected{/if}>Не розподілено</li>
                       <option value="4" {if $order->zone == '4'}selected{/if}>Петрівка</li>
                       <option value="5" {if $order->zone == '5'}selected{/if}>Заболотного</li>
                       <option value="6" {if $order->zone == '6'}selected{/if}>Проліски</li>
                    </select>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-lg-12">
                      <button type="submit" id="submitKievChange" name="submitKievChange" class="btn btn-default pull-right">
                        <i class="icon-save"></i>
                        {l s='Изменить' d='Admin.Actions'}
                      </button>
                    </div>
                  </div>
                </form>
              ...
              {/if}
            {if ($order->id_shop_group == 1 && $order->id_shop == 1 && ($employee == 1 || $employee == 29))}
            <div class="panel panel-sm">
                <div class="panel-heading">
                  <i class="icon-change"></i>
                  {l s='Обновление водителя в ЦБ' d='Admin.Orderscustomers.Feature'}
                </div>
                <form id="change_filial" class="form-horizontal" method="post" action="{$link->getAdminLink('AdminOrders', true, [], ['vieworder' => 1, 'id_order' => $order->id|intval])|escape:'html':'UTF-8'}">
                  <div class="form-group">
                  ...
                  <div class="row">
                    <div class="col-lg-12">
                      <button type="submit" id="submitVodilaChange" name="submitVodilaChange" class="btn btn-default pull-right">
                        <i class="icon-save"></i>
                        {l s='Обновить' d='Admin.Actions'}
                      </button>
                    </div>
                  </div>
                </form>
            ...
            <div class="tab-pane  in active" id="addressShipping">
              ...
              <!-- Shipping address -->
                ...
                <div class="well">
                  <div class="row">
                    <div class="col-sm-12">
                      <a class="btn btn-default pull-right" href="?tab=AdminAddresses&amp;id_address={$addresses.delivery->id}&amp;updateaddress&amp;token={getAdminToken tab='AdminAddresses'}&amp;back={$smarty.server.REQUEST_URI|urlencode}">
                        <i class="icon-pencil"></i>
                        {l s='Edit' d='Admin.Actions'}
                      </a>
                      {$addresses.delivery->lastname} {$addresses.delivery->firstname} <br />
                      {if $addresses.delivery->company}{$addresses.delivery->company}{/if}<br />
                      {$addresses.delivery->city}<br />
                      {$addresses.delivery->address1}<br />
                      {$addresses.delivery->phone_mobile}<br />
                      {$addresses.delivery->phone}<br />
                      {assign var=adr_array value="|"|explode:$addresses.delivery->address2}
											{if $addresses.delivery->valid_adr == 1}
												<hr /><span style="color:green;font-weight:bold;">{l s='Адрес проверен' d='Admin.Actions'}</span><br />
											{else}
                      <hr /><span style="color:red;font-weight:bold;">{l s='Адрес не проверен' d='Admin.Actions'}</span><br />
                      {/if}
                      {if $addresses.delivery->zone}
                        <hr /><span style="color:green;font-weight:bold;">{l s='Зона геолокации' d='Admin.Actions'}: {$addresses.delivery->zone_name} </span><br />
                      {/if}
    ...
    <!--      <div class="panel panel-highlighted">
        ...
        {/if}
        {if (sizeof($customer_thread_message))}
					<div class="panel panel-highlighted">
						<div class="message-item">
							{foreach from=$customer_thread_message item=message}
								<div class="message-avatar">
									<div class="avatar-md">
										<i class="icon-user icon-2x"></i>
									</div>
            ...
            <table class="table" id="orderProducts">
              <thead>
                <tr>
                  ...
                  <th  {if ($order->id_shop_group == 2)}style="width:170px;"{/if}>
                  ...
                  <th><span class="title_box ">{l s='Фасовка' d='Admin.Orderscustomers.Feature'}</span></th>
                  <th class="text-center"><span class="title_box ">{l s='Собрано' d='Admin.Orderscustomers.Feature'}</span></th>
                  <th class="text-center"><span class="title_box ">{l s='Available quantity' d='Admin.Orderscustomers.Feature'}</span></th>
                  <th>
