	...
	<div class="col-lg-6">
		<div class="panel">
			...
			{else}
				...
				{else}
          <input type="text" id="cust_id" name="cust_id" value="" placeholder="Введите номер клиента" style="width: 25%;float: left;">
          <button type=button" class="btn btn-default" id="cust_id_b" name="cust_id_b" value=""><i class="icon-pencil"></i>Привязать</button>
        {/if}
			{/if}
		</div>
	</div>
...
<script>
      $( document ).ready(function() {
        ...
        $('#cust_id_b').click(function() {
           window.location.href = url+'&ajaxchange=1&user_id='+$('#cust_id').val();
        });
