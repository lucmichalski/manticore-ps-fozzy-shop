...
class FsRedirectMessenger
{
    ...
    public static function getSuccessMessages($html = false)
    {
        ...
        if ($html) {
            if ($return_messages) {
                $module = Module::getInstanceByName('fsredirect');
    ...
    private static function readFromCookie()
    {
        ...
        if (!self::$readed_from_cookie) {
            if (isset($cookie->fsmessenger)) {
                self::$messages = Fsredirect::jsonDecodeStatic($cookie->fsmessenger, true);
    ...
    {
        $cookie = Context::getContext()->cookie;
        $cookie->fsmessenger = Fsredirect::jsonEncodeStatic(self::$messages);
    }
}
