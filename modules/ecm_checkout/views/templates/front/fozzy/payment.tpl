 ...
 * @category  Module
*}
<span class="step_number step_number_5">4</span>
  ...
  <div class="payment-options {if $is_free}hidden-xs-up{/if}">
    {foreach from=$payment_options item="module_options"}
      {foreach from=$module_options item="option"}
        ...
        {/if}
          <div id="{$option.id}-container" class="sc_payment_row payment-option clearfix {if !Configuration::get('ecm_checkout_show_radio') &&  $option.id === $selected_payment_option}sc_payment_row_checked{/if}">
		...
		{if !Configuration::get('ecm_checkout_simple_name_pay') &&  $option.id === $selected_payment_option}
			{if  $option.sc_description}
			  <div id="{$option.id}-additional-information" class="sc_payment_row {if !Configuration::get('ecm_checkout_show_radio') &&  $option.id === $selected_payment_option}sc_payment_row_checked{/if}">
