...
<span class="step_number step_number_5">5</span>
<div class="card-block">
    <h5 class="h5">{l s='Summary' mod='ecm_checkout'}</h5>
	<hr class="separator">
</div>
...
<section class="card-block">
	...
	<p class="alert alert-warning" id="discount_error" style="display: none;"></p>
        ...
        <div class="card-block">
            ...
            <div class="cart-summary-line" id="cart-subtotal-{$subtotal.type}">
                <span class="label{if 'products' === $subtotal.type} js-subtotal{/if}">
                    {if 'products' == $subtotal.type}{l s='Всего за товары' mod='ecm_checkout'}{else}{$subtotal.label}{/if}
                </span>
                <span class="value">
        ...
        {if !Configuration::get('ecm_checkout_hide_discount') && $cart.vouchers.allowed}
            ...
            <div class="cart-voucher">
                ...
                <ul class="promo-name card-block">
                    ...
                    <li class="cart-summary-line">
                        ...
                        <a onclick = "action('delete_Discount', '{$voucher.id_cart_rule}')"
                ...
                <div class="promo-code" id="promo-code">
                    <input class="promo-input col col-7 form-control" type="text" id="discount_name" placeholder="{l s='Promo code' d='Shop.Theme.Checkout'}"/>
                    <button onclick = "add_Discount()" class="btn btn-primary  col col-4">
                        <span>
        ...
        <div class="card-block">
            <div class="cart-summary-line cart-total clearfix">
                <span class="label">
            ...
            <div class="float-xs-left">
                <span class="custom-checkbox">
                    <input class="oncheck" type="checkbox" name="callme" id="callme" value="1"
                    act="save_address" {if $callme == 1}checked="checked"{/if}/>
                    <span><i class="material-icons rtl-no-flip checkbox-checked">&#xE5CA;</i></span>
                    <label for="callme">{l s='Call me please.' mod='ecm_checkout'}</label>
                </span>
            ...
            <div id="gift" class="collapse {if $gift.isGift}in{/if}">
                <label for="gift_message">
                    {l s="If you'd like, you can add a note to the gift:" mod='ecm_checkout'}
                </label>
    ...
    <div class="checkout cart-detailed-actions card-block">
        ...
        {else}
        <div class="text-sm-center">
            <p onclick = "to_checkout()" class="btn btn-primary">
                {l s='Proceed to checkout' d='Shop.Theme.Actions'}
            </p>
...
function add_checkout_event(){
    ...
    $(".voucher_name").off('click')
    $(".voucher_name").on('click', function(e){
		action('add_Discount', $(this).data('code'));
	});
    ...
    function to_checkout(){
		...
		$.each($('.sc_customer input'), function(index, value) {
			if(typeof($(value).parent().attr('hidden')) != 'string' && !$(value).hasClass('customer_oncheck')){
        ...
        $.each($('#ecm_checkout input:required'), function(index, value) {
			...
			if (showerror){
				var message = ($(value).prop('type') == 'radio' || $(value).prop('type') == 'checkbox')?check_error:fill_error;
				$.growl.error({
		...
		if(verify_password && !password_generate && type_auth == 'registration' && $('#password').val() !== $('#password2').val()){
			noerror = false;
			$('#password2').removeClass('sc-ok').addClass('sc-error');
			$.growl.error({
				title: $('#password2').attr('title'),
				size: "medium",
		...
		if (noerror) {
            if (type_auth == 'registration' && !password_generate) {
