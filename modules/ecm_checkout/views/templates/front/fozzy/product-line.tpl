    ...
    <div class="col-12 col-sm-6 col-md-6 product-line-grid-right product-line-actions">
        <div class="row align-items-center small-gutters justify-content-end">
          ...
          <div class="col col-auto col-md qty">
            ...
            {else}
              <a id="btn_change_pagecart_{$product.id_product}" 
                       data-product="{$product.id_product}" 
                       class="btn btn-product-list in-page-cart" 
                       data-toggle="modal" 
                       data-target="#blockcart-modal-{$product.id}">
                    <span class="down-inbutton"><i class="fa fa-minus cbp-iconbars"></i></span>
                    {if $product.minimal_quantity == '0.1000'}
                      {if $product.quantity < 1}
                        <span class="button_qty_in_wrapper"><span class="button_qty_in">{$product.quantity*1000|string_format:"%.2f"}</span> гр</span>
                      {else}
                        <span class="button_qty_in_wrapper"><span class="button_qty_in">{$product.quantity|string_format:"%.2f"}</span> кг</span>
                      {/if}
                    {else}
                    <span class="button_qty_in_wrapper"><span class="button_qty_in">{$product.quantity|string_format:"%d"}</span> шт</span>
                    {/if}
                    <span class="up-inbutton"><i class="fa fa-plus cbp-iconbars"></i></span>
                    </a>
              <div class="input-group bootstrap-touchspin">
              <span class="input-group-addon bootstrap-touchspin-prefix" style="display: none;"></span>
              
              <input class="form-control cart_quantity_input" 
				...
				prod_attr = "{$product.id_product|escape:'javascript'}_{$product.id_product_attribute|escape:'javascript'}"
            ...
            <div class="col col-auto">
               <div class="cart-line-product-actions">
                ...
                <a  class="remove-from-cart" rel="nofollow" href="#" 
                  title="{l s='Delete'  mod='ecm_checkout'}"
                  onclick="action('quantity_delete', '{$product.id_product|escape:'javascript'}_{$product.id_product_attribute|escape:'javascript'}')">
