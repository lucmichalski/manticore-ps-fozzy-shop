...
{*$sc_customer*}
<div class="card-block"><h5 class="h5"><i class="material-icons mi-account_circle">account_circle</i> 
...
{/if}
<div class="required form-row"  {if $authMethod == 1}hidden{/if} >
        <label for="email" {if $authMethod == 0}class="required-after"{/if}>{l s='E-mail' mod='ecm_checkout'}</label>
...
<div class="required form-row">
        <label for="{$item}" class="required-after">{l s='Login phone' mod='ecm_checkout'}</label>
...
$(".customer_oncheck").on("change", function (){
...
$('.type_auth').on ('change', function(){
	...
	if (type_auth != 'guest') {
		...
		if ($('#password')[0].checkValidity()) {
			$('#password').removeClass('sc-error').addClass('sc-ok');
		} else {
			$('#password').removeClass('sc-ok').addClass('sc-error');
		}
	} else {
		$('.password').prop('hidden',true);
		$('#password').removeAttr('required');
		$('#password').removeClass('sc-error');
