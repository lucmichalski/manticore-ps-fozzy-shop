...
<div class="card-block"><h5 class="h5"><i class="fa fa-money"></i> {l s='Payment' mod='ecm_checkout'}</h5></div>
<hr class="separator">
  ...
  <div class="payment-options {if $is_free}hidden-xs-up{/if}">
    {foreach from=$payment_options item="module_options"}
      {foreach from=$module_options item="option"}
        <div  class="_row _sc_payment_row {if $selected_payment_option == $option.id || $is_free} sc_payment_checked{/if}" >
...
$(".sc_payment_row").on("click", function (){
	...
	action('set_payment', $(this).attr('value'));
});
