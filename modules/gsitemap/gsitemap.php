...
class Gsitemap extends Module
{
    ...
    public function addLinkToSitemap(&$link_sitemap, $new_link, $lang, &$index, &$i, $id_obj)
    {
        if ($i <= 25000 && memory_get_usage() < 100000000) {
            ...
            if ($index % 20 == 0 && !$this->cron) {
                ...
                header('Refresh: 5; url=http' . (Configuration::get('PS_SSL_ENABLED') ? 's' : '') . '://' . Tools::getShopDomain(false, true) . '/modules/gsitemap/gsitemap-cron.php?continue=1&token=' . Tools::substr(Tools::encrypt('gsitemap/cron'), 0, 10) . '&type=' . $new_link['type'] . '&lang=' . $lang . '&index=' . $index . '&id=' . (int) $id_obj . '&id_shop=' . $this->context->shop->id);
                die();
            } else {
                if ($this->cron) {
                    $s_url = 'http' . (Configuration::get('PS_SSL_ENABLED') ? 's' : '') . '://' . Tools::getShopDomain(false, true);
                 //   Tools::redirect($this->context->link->getBaseLink() . 'modules/gsitemap/gsitemap-cron.php?continue=1&token=' . Tools::substr(Tools::encrypt('gsitemap/cron'), 0, 10) . '&type=' . $new_link['type'] . '&lang=' . $lang . '&index=' . $index . '&id=' . (int) $id_obj . '&id_shop=' . $this->context->shop->id);
                    Tools::redirect($s_url . '/modules/gsitemap/gsitemap-cron.php?continue=1&token=' . Tools::substr(Tools::encrypt('gsitemap/cron'), 0, 10) . '&type=' . $new_link['type'] . '&lang=' . $lang . '&index=' . $index . '&id=' . (int) $id_obj . '&id_shop=' . $this->context->shop->id);
                } else {
    ...
    protected function recursiveSitemapCreator($link_sitemap, $lang, &$index)
    {
        ...
        $sitemap_link = 'shop_' . $this->context->shop->id . '_' . $lang . '_' . $index . '_sitemap.xml';
    ...
    protected function createIndexSitemap()
    {
        ...
        file_put_contents($this->normalizeDirectory(_PS_ROOT_DIR_) . 'shop_' .$this->context->shop->id . '_index_sitemap.xml', $xml_feed->asXML());
