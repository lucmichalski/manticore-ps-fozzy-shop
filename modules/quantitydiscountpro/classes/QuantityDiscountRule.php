    ...
    public static function getQuantityDiscountRulesByFamily($id_family = false, $codes = false, $highlight = false)
    {
        ...
        if (!Cache::isStored($cache_key)) {
            $sql = "SELECT *
                ...
                LEFT JOIN `"._DB_PREFIX_."quantity_discount_rule_lang` qdrl
        ...
        if (flock($file, LOCK_EX | LOCK_NB)) {
            ...
            */
            ...
            if (!$this->array_search_partial(array_column($backtrace, 'file'), 'hsmultiaccessoriespro')) {
                ...
                foreach (array('carriercompare', 'productadditionalfeatures', 'attributegrid', 'imaximprimepedidosservidor', 'appagebuilder', 'phcarrito') as $modules_exception) {
        ...
        foreach ($actions as $action) {
            switch ((int)$action->id_type) {
                ...
                case 1:
                    ...
                    $carrierList = $this->context->cart->getDeliveryOption(null, false, false);
                    ...
                    foreach ($carrierList as $carrier) {
                        ...
                        if ($shippingCost == $action->reduction_amount) {
                            ...
                            continue;
                        } elseif ($orderTotal >= $shippingCost) {
                    ...
                    if (!$tempCartRule[$tempCartRuleCounter]['reduction_amount']
                        && !$tempCartRule[$tempCartRuleCounter]['free_shipping']) {
                ...
                case 5:
                    ...
                    if ((int)$action->reduction_percent == 100
                        && !(int)$action->reduction_max_amount
                        && !$action->filter_by_carrier) {
                        $tempCartRule[$tempCartRuleCounter]['free_shipping'] = 1;
                        break;
                    }
                    ...
                    if ($action->filter_by_carrier
                        && count($carrierList) == 1
                        && array_column($restrictionCarriers['selected'], $carrierList[0])) {
                        $tempCartRule[$tempCartRuleCounter]['free_shipping'] = 1;
                        break;
                    }
                ...
                case 101:
                    ...
                    for ($i = 0; $i < $minCoincidences; $i++) {
                        ...
                        if ($unitsRemoved) {
                            ...
                            foreach (array_reverse($productsGrouped) as &$productGrouped) {
                                if ((int)$productGrouped['cart_quantity'] >= (int)$action->apply_discount_to_nb) {
