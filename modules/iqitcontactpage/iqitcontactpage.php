...
class IqitContactPage extends Module implements WidgetInterface
{
        ...
        if (Tools::isSubmit('submitModule')) {
            foreach ($this->defaults as $default => $value) {
                if ($default == 'content') {
                    ...
                    $message_trads = array();
                    foreach ($_POST as $key => $value) {
                        if (preg_match('/address_/i', $key)) {
                            $id_lang = preg_split('/address_/i', $key);
                            $message_trads[(int)$id_lang[1]] = $value;
                        }
                    }
                    Configuration::updateValue($this->cfgName . $default, $message_trads, true);
                }
                else if ($default == 'company') {
                    $message_trads = array();
                    foreach ($_POST as $key => $value) {
                        if (preg_match('/company_/i', $key)) {
                            $id_lang = preg_split('/company_/i', $key);
                            $message_trads[(int)$id_lang[1]] = $value;
                        }
                    }
                    Configuration::updateValue($this->cfgName . $default, $message_trads, true);
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        if (preg_match('/^displayContactMap\d*$/', $hookName)) {
            ...
            return array(
                'company' => Configuration::get($this->cfgName . 'company', $this->context->language->id),
                'address' => Configuration::get($this->cfgName . 'address', $this->context->language->id),
                'phone' => Configuration::get($this->cfgName . 'phone'),
                'phone2' => Configuration::get($this->cfgName . 'phone2'),
                'phone3' => Configuration::get($this->cfgName . 'phone3'),
