    ...
    {block name="td_content"}
            ...
            {if isset($tr[$key])}
                <span class="label-tooltip" data-original-title="{l s='Click here to see faq category on your site' mod='blockfaq'}" data-toggle="tooltip">
                    ...
                    {if $params.is_rewrite == 0}
                        {$link->getModuleLink('blockfaq', 'faq', [], true, {$tr.id_lang|escape:'htmlall':'UTF-8'}, {$tr.id_shop|escape:'htmlall':'UTF-8'})|escape:'htmlall':'UTF-8'}?category_id={$tr['id']|escape:'htmlall':'UTF-8'}
                    {else}
                        {$params.base_dir_ssl|escape:'htmlall':'UTF-8'}{$params.iso_code|escape:'htmlall':'UTF-8'}faq?category_id={$tr['id']|escape:'htmlall':'UTF-8'}
                    {/if}
                    "
        ...
        {elseif isset($params.type_custom) && $params.type_custom == 'is_active'}
            ...
            <span id="activeitem{$tr['id']|escape:'htmlall':'UTF-8'}">
                    ...
                    <a href="javascript:void(0)" onclick="blockfaq_list({$tr['id']|escape:'htmlall':'UTF-8'},'active',{$tr[$key]|escape:'htmlall':'UTF-8'},'categoryfaq');" style="text-decoration:none">
        ...
        {elseif isset($params.type_custom) && $params.type_custom == 'order_by'}
            ...
            <a class="btn btn-default"
              href="index.php?controller={$params.name_controller|escape:'htmlall':'UTF-8'}&token={$params.token|escape:'htmlall':'UTF-8'}&id_faqcat={$tr.id|escape:'htmlall':'UTF-8'}&order_self_faqcat={$tr.order_by|escape:'htmlall':'UTF-8'}&id_change_faqcat={$params.all_items_sort[$index+1].id|escape:'htmlall':'UTF-8'}&order_change_faqcat={$params.all_items_sort[$index+1].order_by|escape:'htmlall':'UTF-8'}"
             >
