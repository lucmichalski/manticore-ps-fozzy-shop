...
class Ps_Viewedproduct extends Module implements WidgetInterface
{
    ...
    {
        if ($this->context->controller->page_name == "module-ecm_checkout-checkout")
        {
          return;
        }
        ...
        $variables['hook'] = $hookName;
        $variables['options_s'] ='{"slidesToShow":5,"slidesToScroll":5,"slidesToShowTablet":4,"slidesToShowMobile":2,"itemsPerColumn":1,"autoplay":false,"infinite":true,"arrows":true,"dots":false}';
       ...
       //  die();
            ...
            if (is_array($productIds)) {
                foreach ($productIds as $productId) {
                    $product = new Product($productId,true,$this->context->language,$this->context->shop->id);
                    $product_a = array();
                    $product_a = (array)$product;
                    $product_a['id_product'] = $productId;
                    if ($this->currentProductId !== $productId) {
                        $products_for_template[] = $presenter->present(
                            $presentationSettings,
                            $assembler->assembleProduct($product_a),
