...
class SearchCore
{
    ...
    public static function find(
        ...
        $results = $db->executeS('
		SELECT SQL_CACHE DISTINCT cp.`id_product`
        ...
        foreach ($intersect_array as $keys=>$query) {
            foreach ($db->executeS($query, true, false) as $row) {
                $eligible_products2[] = $row['id_product'];
            }
            if($keys == 0) {
                  $eligible_products3 = $eligible_products2;
                  $eligible_products2 = array();
            }
            else {
                  $eligible_products3 = array_intersect($eligible_products3,$eligible_products2);
                  $eligible_products2 = array();
            }
        }
        $eligible_products = array_unique(array_intersect($eligible_products, array_unique($eligible_products3)));
