...
abstract class ProductListingFrontControllerCore extends ProductPresentingFrontController
{
    ...
    protected function getProductSearchVariables()
    {
        ...
        $products_do = $result->getProducts();
    //    if ($this->context->customer->id == 5)
    //    {
         foreach ($products_do as $key_p=>$product)
         {                 
          $sup=array();
          $sql_supp = "SELECT p.`id_supplier` FROM `" . _DB_PREFIX_ . "product_supplier` p, `" . _DB_PREFIX_ . "supplier` s WHERE p.`id_product` = ".$product['id_product']." AND p.`id_supplier` = s.`id_supplier` AND s.`active` = 1";
          $suppliers = Db::getInstance()->executeS($sql_supp);
          
          foreach ($suppliers as $supplier) {
            $sup[]=(int)$supplier['id_supplier'];
          }
          $products_do[$key_p]['suppliers']=$sup;
         }
    //    }
        ...
        foreach ($products_do as $key_p=>$product)
         {                 
          $prices_im = array();
          $sql_prices_im = "SELECT * FROM `"._DB_PREFIX_."fozzy_autoupdate_online` WHERE `id_product` = ".$product['id_product']." AND (NOW() BETWEEN `date_start` AND `date_stop`)";
          $prices_im = Db::getInstance()->executeS($sql_prices_im);
          $prices_im_act = count($prices_im);
           
            if ($prices_im_act)
              {
                foreach ($prices_im as $price_im) {
                $products_do[$key_p]['price_retail']=$price_im['price_retail'];
                }
              }
            else
              {
               $products_do[$key_p]['price_retail']=0;
              }
          }
         /*   if ($this->context->customer->id == 5)
            {
        ...
        $searchVariables = array(
            ...
            'id_shop' => (int)$this->context->shop->id,       //Britoff
            'id_shop_group' => (int)$this->context->shop->id_shop_group,  //Britoff
            'products' => $products,
