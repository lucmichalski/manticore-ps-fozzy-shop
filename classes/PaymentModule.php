...
abstract class PaymentModuleCore extends Module
{
    ...
    public function validateOrder(
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            */ //Britoff - Убрал статус - Нет в наличии
                    unset($order_detail);
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        $cart = Db::getInstance()->executeS($sql_select);
                            ...
                            if (count($cart) > 0)
                              ...
                              $errortext = Tools::displayError('С')." ".$links[0]['timefrom']." ".Tools::displayError('по')." ".$links[0]['timeto']; 
                        ...
                        if (Validate::isEmail($this->context->customer->email) && (int)$id_order_state != 939) {
                            Mail::Send(
                                (int) $order->id_lang,
                                'order_conf',
                                Context::getContext()->getTranslator()->trans(
