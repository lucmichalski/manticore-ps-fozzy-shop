...
abstract class HTMLTemplateCore
{
    ...
    public function assignCommonHeaderData()
    {
        ...
        //Britoff
        $ship_cost=(int)$this->order->total_shipping;
        $discounts = $this->order->getCartRules();
        $carrier = (int)$this->order->id_carrier; //перевозчик
        $courier_array = array();
        $courier_array = array(27,33,30,42);
        ...
        $this->title = "Заказ № ".$this->order->id;
        
        //CODE128 Text
        $ean_dost = 0;
        if ($shipprint > 0 ) $ean_dost = '600757';
        $code128 = '1167615_'.$this->order->id.'_'.$ean_dost;
        
        //Britoff
        
        $this->smarty->assign(array(
            'logo_path' => $path_logo,
            'code128' => $code128, // Britoff - code128
            'shipprint' => $shipprint,  // Britoff - Карта покупателя
