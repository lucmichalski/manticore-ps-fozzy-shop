...
class HTMLTemplateInvoiceCore extends HTMLTemplate
{
    ...
    public function getContent()
    {
        ...
        // Britoff - Добавляем категории к списку товаров
        ...
        $order_details = Tools::ArrayOrderBy($order_details, 'parent_category', SORT_ASC, 'product_name', SORT_ASC);
        unset($order_detail);
        // Britoff - Добавляем категории к списку товаров
        ...
        //Britoff
        $messages = CustomerMessage::getMessagesByOrderId((int)$this->order->id);    // Britoff - Сообщение покупателя

        $delivery_other=$delivery_address->other;         //Britoff - Доп данные по адресу
        $delivery_phone=$delivery_address->phone;         //Britoff - телефоны доставки
        $delivery_mphone=$delivery_address->phone_mobile;   //Britoff - телефоны доставки
        $delivery_adrress_pdf = $delivery_address->address1; //Britoff - адрес полный
        $delivery_adrress_city = $delivery_address->city; //Britoff - адрес город
                
        $delivery_adrress_company = $delivery_address->company;
        $delivery_adrress_name = $delivery_address->firstname." ".$delivery_address->lastname;

        $del_ad_ar = array();
        $del_ad_ar = explode('|',$delivery_address->address2);
        $delivery_adrress_pdf_short = $del_ad_ar[0].", ".$del_ad_ar[1];  //Britoff - адрес короткий
        ...
        $data = array(
            ...
            'cart1' => $cart1,
            'carrier' => $carrier,
            'shipping' => $carrier_name, // Britoff - Доставка
            'shipping_all' => $shipping,   // Britoff - Доставка
            'messages' => $messages, // Britoff - Сообщение покупателя
            'cart_rules' => $cart_rules,
            'delivery_address' => $formatted_delivery_address,
            'delivery_phone' => $delivery_phone, //Britoff - телефоны доставки
            'delivery_other' => $delivery_other, //Britoff - Доп данные по адресу
            'delivery_mphone' => $delivery_mphone, //Britoff - телефоны доставки
            'delivery_adrress_city' => $delivery_adrress_city, //Britoff - телефоны город
            'delivery_adrress_company' => $delivery_adrress_company, //Britoff - телефоны доставки
            'delivery_adrress_name' => $delivery_adrress_name, //Britoff - телефоны доставки
            'delivery_adrress_pdf' => $delivery_adrress_pdf, //Britoff - адрес полный
            'delivery_adrress_pdf_short' => $delivery_adrress_pdf_short, //Britoff - адрес короткий
