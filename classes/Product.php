...
class ProductCore extends ObjectModel
{
    ...
    /** @var float custom procies */
    public $mopt_price = null;  //Мелкооптовая цена
    public $z_price = null;    //Цена закупки
    public $mrс = null;       //МРЦ
    public $opt_kol = null;   //Оптовое количество
    public $mopt_kol = null;  //Мелкооптовое количество
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'minimal_quantity' => array('type' => self::TYPE_FLOAT, 'shop' => true, 'validate' => 'isUnsignedFloat'),
            ...
            'mopt_price' => array('type' => self::TYPE_FLOAT, 'shop' => true, 'validate' => 'isPrice'),
            'z_price' => array('type' => self::TYPE_FLOAT, 'shop' => true, 'validate' => 'isPrice'),
            'mrс' => array('type' => self::TYPE_FLOAT, 'shop' => true, 'validate' => 'isPrice'),
            'opt_kol' => array('type' => self::TYPE_FLOAT, 'shop' => true, 'validate' => 'isPrice'),
            'mopt_kol' => array('type' => self::TYPE_FLOAT, 'shop' => true, 'validate' => 'isPrice'),
            ...
            /* Lang fields */
            'meta_description' => array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isGenericName', 'size' => 2048),
            'meta_keywords' => array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isGenericName', 'size' => 255),
            'meta_title' => array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isGenericName', 'size' => 1024),
            'link_rewrite' => array(
                'type' => self::TYPE_STRING,
                'lang' => true,
    ...
    public static function getDefaultAttribute($id_product, $minimum_quantity = 0, $reset = false)
    {
        ...
        $sql = 'SELECT product_attribute_shop.id_product_attribute
                ...
                . ($minimum_quantity > 0 ? ' AND IFNULL(stock.quantity, 0) >= ' . (float) $minimum_quantity : '') .
    ...
    public function updateAttribute(
        ...
        $combination->minimal_quantity = (float) $minimal_quantity;
    ...
    public static function getAllCustomizedDatas($id_cart, $id_lang = null, $only_in_cart = true, $id_shop = null, $id_customization = null)
    {
        ...
        if ($id_customization === 0) {
            ...
            $product_customizations = (int) Db::getInstance()->getValue('
                SELECT SQL_CACHE COUNT(`id_customization`) FROM `' . _DB_PREFIX_ . 'cart_product`
        ...
        if (!$result = Db::getInstance()->executeS('
            SELECT SQL_CACHE cd.`id_customization`, c.`id_address_delivery`, c.`id_product`, cfl.`id_customization_field`, c.`id_product_attribute`,
        ...
        if (!$result = Db::getInstance()->executeS(
            'SELECT SQL_CACHE `id_product`, `id_product_attribute`, `id_customization`, `id_address_delivery`, `quantity`, `quantity_refunded`, `quantity_returned`
        ...
        foreach ($result as $row) {
            $customized_datas[(int) $row['id_product']][(int) $row['id_product_attribute']][(int) $row['id_address_delivery']][(float) $row['id_customization']]['quantity'] = (float) $row['quantity'];
            $customized_datas[(int) $row['id_product']][(int) $row['id_product_attribute']][(int) $row['id_address_delivery']][(float) $row['id_customization']]['quantity_refunded'] = (float) $row['quantity_refunded'];
            $customized_datas[(int) $row['id_product']][(int) $row['id_product_attribute']][(int) $row['id_address_delivery']][(float) $row['id_customization']]['quantity_returned'] = (float) $row['quantity_returned'];
