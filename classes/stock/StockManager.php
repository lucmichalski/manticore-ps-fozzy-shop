...
class StockManagerCore implements StockManagerInterface
{
    ...
    public function removeProduct(
        ...
        if (Pack::isPack((int) $id_product) && !$ignore_pack) {
            ...
            $physical_quantity_in_stock = (float) $this->getProductPhysicalQuantities($id_product, $id_product_attribute, array($warehouse->id), false);
            $usable_quantity_in_stock = (float) $this->getProductPhysicalQuantities($id_product, $id_product_attribute, array($warehouse->id), true);
                    ...
                    foreach ($stock_collection as $stock) {
                        ...
                        while ($row = Db::getInstance()->nextRow($resource)) {
                            ...
                            if ($warehouse->management_type == 'LIFO') {
                                $left_quantity_to_check -= (float) $row['qty'];
                                if ($left_quantity_to_check <= 0) {
                                    break;
                                }
                            }
