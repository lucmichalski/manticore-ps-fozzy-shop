...
class StockAvailableCore extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'quantity' => array('type' => self::TYPE_FLOAT, 'validate' => 'isFloat', 'required' => true),
    ...
    public static function setQuantity($id_product, $id_product_attribute, $quantity, $id_shop = null, $add_movement = true)
    {
        ...
        if (!$depends_on_stock) {
            ...
            $id_stock_available = (float) StockAvailable::getStockAvailableIdByProductId($id_product, $id_product_attribute, $id_shop);
            if ($id_stock_available) {
                $stock_available = new StockAvailable($id_stock_available);

                $deltaQuantity = -1 * ((float) $stock_available->quantity - (float) $quantity);

                $stock_available->quantity = (float) $quantity;
                ...
                if (true === $add_movement && 0 != $quantity) {
                    $stockManager->saveMovement($id_product, $id_product_attribute, (float) $quantity);
                }
            }
