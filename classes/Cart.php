...
class CartCore extends ObjectModel
{
    ...
    public function getProducts($refresh = false, $id_product = false, $id_country = null, $fullInfos = true)
    {
        ...
        //Britoff - FozzySampling Start -----------
        
        $temp_p = $this->_products;
        $gift_array = array(57195,57194,57193,57192,57191,57190,57189,57188,57187,57186,46059,41473,35594,34235,34234);
          foreach ($temp_p as $g_key=>$g_p)
            {
            // if ($g_p['id_product'] == 57190)  $temp_p[$g_key]['legend'] = "fozzy_gift";
           //  if ($g_p['id_product'] == 57194)  $temp_p[$g_key]['legend'] = "fozzy_gift";
           //  if ($g_p['id_product'] == 57195)  $temp_p[$g_key]['legend'] = "fozzy_gift";
             if (in_array($g_p['id_product'],$gift_array))  $temp_p[$g_key]['legend'] = "fozzy_gift";
             
         //    Изменяем легенду изображения на fozzy_gift для товаров семпла.
            } 
        $this->_products = $temp_p;
        
       //Britoff - FozzySampling  End --------------------
    ...
    protected function applyProductCalculations($row, $shopContext, $productQuantity = null)
    {
        ...
        $row['quantity_discount_applies'] = ($specific_price_output && $productQuantity >= (float) $specific_price_output['from_quantity']);
    ...
    public function updateQty(
        ...
        if ($cartProductQuantity['quantity'] == 0) $cartProductQuantity['quantity'] = null;     //Britoff - очень пиздец какой важный фикс для добавления товара!
        ...
        if (!empty($cartProductQuantity['quantity'])) {
            ...
            if ($operator == 'up') {
                ...
                if ($cartFirstLevelProductQuantity['quantity'] <= 0.01
    ...
    protected function _updateCustomizationQuantity($quantity, $id_customization, $id_product, $id_product_attribute, $id_address_delivery, $operator = 'up')
    {
        ...
        /* Deletion */
        if (!empty($id_customization) && (float) $quantity < 0) {
        ...
        if (!empty($id_customization)) {
            ...
            if ($result && Db::getInstance()->numRows()) {
                if ($operator == 'down' && (float) $result['quantity'] - (float) $quantity < 0) {
    ...
    public function deleteProduct(
        ...
        if ($preservedGifts[(int) $id_product . '-' . (int) $id_product_attribute] > 0) {
            return Db::getInstance()->execute(
                'UPDATE `' . _DB_PREFIX_ . 'cart_product`
                SET `quantity` = ' . (float) $preservedGifts[(int) $id_product . '-' . (int) $id_product_attribute] . '
                WHERE `id_cart` = ' . (int) $this->id . '
                AND `id_product` = ' . (int) $id_product .
                ($id_product_attribute != null ? ' AND `id_product_attribute` = ' . (int) $id_product_attribute : '')
            );
