...
class ProductController extends FrameworkBundleAdminController
{
    ...
    public function listAction(
        ...
        $cdn = Configuration::get('PS_MEDIA_SERVER_1');
        if ($cdn)
        foreach ($products as $key=>$prod)
          {
            $pat_i = '';
            $pat = str_split($prod['id_image']);
            foreach ($pat as $pi)
              {
               $pat_i .= "/".$pi;
              }
            $products[$key]['image'] = '<img src="https://'.$cdn.'/img/p'.$pat_i.'/'.$prod['id_image'].'-small_default.jpg" alt="" class="imgm img-thumbnail" />';
          }
       // dump($products);
      //  die();
        // Template vars injection
